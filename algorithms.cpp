...
void find_test(){
    ...
    if(loc!=vec.end())
    std::cout<<"Found the number at "<<*loc<<std::endl;
    else
...
void count_if_test(){
    ...
    num=std::count_if(vec.begin(),std::end(vec),[](int x){return x>=5;});
    std::cout<<num<<std::endl;
}

//replace occurrence of elements in a container
void replace_test(){
    std::cout<<"\n====================================="<<std::endl;
    std::vector<int> vec{1,2,3,4,5,1,2,1};
    for(auto i:vec)
     std::cout<<i<<" ";
    std::cout<<std::endl;
    std::replace(std::begin(vec),std::end(vec),1,100);
    for(auto i:vec)
     std::cout<<i<<" ";
    std::cout<<std::endl;
}
